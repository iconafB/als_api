...
from schemas.dedupe_campaigns import CreateDedupeCampaign,SubmitDedupeReturnSchema
...
async def add_dedupe_list(camp_code:str,session:Session=Depends(get_session),user=Depends(get_current_user)):
    ...
    try:
        ...
        information_object=[
            information_table(cell_number=data[0],extra_info=data[1]) for data in informatable_data
        ]
...
async def submit_dedupe_return(data:SubmitDedupeReturnSchema,dedupe_file:UploadFile=File(...),session:Session=Depends(get_session)):
    try:
        ...
        #select id from campaign_dedupe where status is 'P' and code matches the supplied code
