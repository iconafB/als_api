...
from schemas.campaign_rules import CreateCampaignRule,ChangeCampaignResponse,CreateDedupeCampaignRule,RuleCreate
...
from crud.campaign_rules import (create_campaign_rule)
campaign_rule_router=APIRouter(tags=["Generic Campaign Rules"],prefix="/campaign-rules")
...
async def create_campaign_rule(req:Request,rule:RuleCreate,session:AsyncSession=Depends(get_async_session)):
    ...
    try:
        ...
        create_rule=await create_campaign_rule(rule,session)
...
async def assign_active_rule_to_campaign(rule_code:int,camp_code:str,session:Session=Depends(get_session),user=Depends(get_current_active_user)):
    try:
        ...
        find_campaign_rule_query=select(campaign_rules_tbl,rules_tbl).join(rules_tbl,campaign_rules_tbl.rule_code==rules_tbl.rule_code).where(campaign_rules_tbl.camp_code==camp_code,campaign_rules_tbl.is_active==True)
...
async def fetch_rule_code(rule_name:str=Path(...,description="Provide the campaign code as the rule name"),session:AsyncSession=Depends(get_async_session),user=Depends(get_current_active_user)):
    try:
...
async def change_rule_name(rule_code:int=Path(...,description="Campaign code or rule name of a campaign"),rule_name:str=Query(...,description="Campaign name or rule name"),session:Session=Depends(get_session),user=Depends(get_current_active_user)):
    try:
...
async def fetch_campaign_rules(page:int=Query(1,ge=10),size:int=Query(10,ge=1,le=100),session:Session=Depends(get_session),user=Depends(get_current_active_user)):
    try:
...
async def assign_campaign_rule_to_campaign(rule_code:int):
    try:
        return True
    except Exception as e:
