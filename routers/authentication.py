...
from schemas.auth import LoginUser,RegisterUser,RegisterUserResponse,ForgotPassword,Token
from utils.auth import verify_password,hash_password,access_token
...
async def register_user(user:RegisterUser,session:Session=Depends(get_session)):
    ...
    new_user=users_table(email=user.email,password=user.password,full_name=user.full_name)
    if not new_user:
...
async def login_user(user:LoginUser,session:Session=Depends(get_session)):
    #index on email
    #find the user using the email 
    login_user=session.exec(select(users_table).where(users_table.email==user.email)).first()
     #return an error if the user is not found
    if not login_user.email == user.email:
    ...
    token=access_token(data={'user_id':user_id},expires_delta=access_token_expires)
    #return the token
